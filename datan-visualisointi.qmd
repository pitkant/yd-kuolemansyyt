...
ggplot(vakivalta_ikaryhmat_2, aes(fill = ika, y=kuolleet_06_24, x=as.numeric(vuosi))) +
...
ggplot(huumekuolemat_ikaryhmat_2, aes(fill = ika, y=kuolleet_06_24, x=as.numeric(vuosi))) +
  geom_bar(position="dodge", stat="identity", color = "black") + 
...
Stacked bar -plotin PANELin sisäisen prosenttiosuuden laskemisen kaava: https://stackoverflow.com/questions/66957975/percentage-labels-for-a-stacked-ggplot-barplot-with-groups-and-facets
...
kuvio4 <- ggplot(kaikki_long, aes(fill = kuolemansyy, y=value, x=as.numeric(vuosi))) +
  geom_bar(position="stack", stat="identity", color = "black") + 
  labs(x = "Vuosi",
       y = "",
       fill = "Kuolemansyy") +
...
kuvio4 <- kuvio4 + geom_text(
    aes(
      label = sprintf(
        '%d\n(%s)',
        after_stat(y),
        pct_format(after_stat(y / bygroup(y, interaction(x, PANEL))))
      )
    ),
    stat = 'identity',
    colour = 'white',
