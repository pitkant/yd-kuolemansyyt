...
Asetus jolla säädellään sitä, haetaanko datat aina uudelleen pxweb-rajapinnasta vai luetaanko ne lokaalista kansiosta. Pxweb-rajapinnan kuormittamisen välttämiseksi datat luetaan lähtökohtaisesti prepared_data -kansiosta, johon ne on tallennettu.

```{r}
tallenna_datasetit <- lataa_datasetit_uudelleen <- FALSE
kirjoita_data_excel_tiedostoihin <- FALSE
tuota_vektorikuviot <- FALSE
tuota_viivakuviot <- FALSE
```

Taulukossa "12d9 -- Huumeisiin kuolleet (B-luokitus) tilaston peruskuolemansyyn, iän ja sukupuolen mukaan, 2006-2024" ([statfin_ksyyt_pxt_12d9](https://pxdata.stat.fi/PXWeb/pxweb/fi/StatFin/StatFin__ksyyt/statfin_ksyyt_pxt_12d9.px)) käytetään Yhteensä-lukua, joka sisältää seuraavat kategoriat:
...
Mikäli huumekuolemia haluttaisiin katsoa pidemmältä ajalta, tulisi käyttää muita tietokantataulukoita. Taulukossa "Kuolleet muuttujina Tilaston peruskuolemansyy (ICD-10, 3-merkkitaso), Ikä, Vuosi, Tiedot ja Sukupuoli" ([statfin_ksyyt_pxt_11be](https://pxdata.stat.fi/PXWeb/pxweb/fi/StatFin/StatFin__ksyyt/statfin_ksyyt_pxt_11be.px)) on mahdollista valmita em. ICD10-koodien perusteella tietoja vuodesta 1998 alkaen.
...
Tilastokeskuksen aineistosta "11s1 -- Keskiväkiluku iän ja sukupuolen mukaan alueittain, 1981-2024" ([statfin_vaerak_pxt_11s1](https://pxdata.stat.fi/PxWeb/pxweb/fi/StatFin/StatFin__vaerak/statfin_vaerak_pxt_11s1.px/)) saadaan tietoja vain vuoteen 1981 asti. Tästä johtuen myöhemmin tässä työkirjassa käytetään itse laskettua keskiväkilukua, mutta valmisaineisto ladataan tässä esimerkinomaisesti muistutuksena siitä, että tällainenkin on olemassa.
...
Eli vuoden 2024 keskiväkiluku saadaan laskemalla yhteen vuoden 2024 väkiluku ja vuoden 2023 väkiluku ja jakamalla tämä kahdella. Käytetään Tilastokeskuksen aineistoa "11rc -- Väestö iän (5-v.) ja sukupuolen mukaan, 1865-2024" ([statfin_vaerak_pxt_11rc](https://pxdata.stat.fi/PxWeb/pxweb/fi/StatFin/StatFin__vaerak/statfin_vaerak_pxt_11rc.px/)).
...
Elinajanodotteet vuosina ovat järjestyksessä pienimmästä iästä suurimpaan, eli vuonna 1986 15-vuotiaiden elinajanodote oli 56.2 vuotta, 16-vuotiaiden 55.23 vuotta jne. Käytetään Tilastokeskuksen aineistoa "12ap -- Kuolleisuus- ja eloonjäämislukuja iän ja sukupuolen mukaan, 1986-2024" ([statfin_kuol_pxt_12ap](https://pxdata.stat.fi/PXWeb/pxweb/fi/StatFin/StatFin__kuol/statfin_kuol_pxt_12ap.px)).
...
itsemurhat_ikaryhmat_2 <- rbind(itsemurhat_ikaryhmat, itsemurhat_ikaryhmat_total) %>% 
  mutate(ika = factor(ika, levels=c("Yhteensä", "15 - 19", "20 - 24", "25 - 29")))
...
```{r, eval = kirjoita_data_excel_tiedostoihin}
...
# Artikkelin kuviot 1-3: PDF-kuvioiden luominen

## Grouped bar -kuvaajat

```{r 1_yd_kuolemansyyt_visualisointi_artikkeliin}
kuvio1 <- ggplot(data = itsemurhat_ikaryhmat_2, aes(fill = ika, y=kuolleet_06_24, x=vuosi)) +
...
kuvio2 <- ggplot(huumekuolemat_ikaryhmat_2, aes(fill = ika, y=kuolleet_06_24, x=vuosi)) +
  geom_bar(position="dodge", stat="identity", color = "black", size = 0.25) + 
  labs(x = "Vuosi",
       y = "",
       fill = "Ikäryhmä") +
...
kuvio3 <- ggplot(vakivalta_ikaryhmat_2, aes(fill = ika, y=kuolleet_06_24, x=vuosi)) +
  geom_bar(position="dodge", stat="identity", color = "black", size = 0.25) + 
  labs(x = "Vuosi",
       y = "",
       fill = "Ikäryhmä") +
...
if (tuota_vektorikuviot) {
  ...
  # EMF plot experimentation
  ggsave(filename = "prepared_plots/kuvio1_13x8.emf", plot = kuvio1, width = 130, height = 80, units = "mm", dpi = 300, scale = 1, device = {function(filename, ...) devEMF::emf(file = filename, ...)})
  ggsave("prepared_plots/kuvio2_13x8.emf", plot = kuvio2, width = 130, height = 80, units = "mm", dpi = 300, scale = 1, device = emf)
  ggsave("prepared_plots/kuvio3_13x8.emf", plot = kuvio3, width = 130, height = 80, units = "mm", dpi = 300, scale = 1, device = emf)
}
```

## Vaihtoehto: Viivakuvaajat

```{r 1_yd_kuolemansyyt_visualisointi_artikkeliin_viivakuvaajat}
kuvio1_viiva <- ggplot(itsemurhat_ikaryhmat_2, aes(group = ika, y=kuolleet_06_24, x=as.numeric(vuosi))) +
...
kuvio2_viiva <- ggplot(huumekuolemat_ikaryhmat_2, aes(group = ika, y=kuolleet_06_24, x=as.numeric(vuosi))) +
  geom_line(aes(color=ika, linetype=ika), linewidth = 1/.pt) + 
  labs(x = "Vuosi",
       y = "",
       color = "Ikäryhmä", linetype = "Ikäryhmä") +
...
kuvio3_viiva <- ggplot(vakivalta_ikaryhmat_2, aes(group = ika, y=kuolleet_06_24, x=as.numeric(vuosi))) +
  geom_line(aes(color=ika, linetype=ika), linewidth = 1/.pt) + 
  labs(x = "Vuosi",
       y = "",
       color = "Ikäryhmä", linetype = "Ikäryhmä") +
...
if (tuota_vektorikuviot & tuota_viivakuviot) {
  ggsave("prepared_plots/extras/kuvio1_viiva_13x8.pdf", plot = kuvio1_viiva, width = 130, height = 80, units = "mm", dpi = 300, scale = 1)
  ggsave("prepared_plots/extras/kuvio2_viiva_13x8.pdf", plot = kuvio2_viiva, width = 130, height = 80, units = "mm", dpi = 300, scale = 1)
  ggsave("prepared_plots/extras/kuvio3_viiva_13x8.pdf", plot = kuvio3_viiva, width = 130, height = 80, units = "mm", dpi = 300, scale = 1)
}
```
...
"Kuviossa mahdollisesti käytetyt värit tulevat näkyviin artikkelin nettiversiossa." Ikäryhmien visualisointiin käytetään tässä väripalettia BuPu, joka on colorblind safe, sequential (ks. lisätietoja ja lisää paletteja: [colorbrewer2.org](https://colorbrewer2.org)).
...
Värilliset kuvat. Sukupuolen visualisointiin käytetään tässä väripalettia Set2 joka on colorblind safe, qualitative (ks. lisätietoja ja lisää paletteja: [colorbrewer2.org](https://colorbrewer2.org)).
...
Kuolemansyiden visualisointiin käytetään tässä väripalettia Dark2 joka on colorblind safe, qualitative (ks. lisätietoja ja lisää paletteja: [colorbrewer2.org](https://colorbrewer2.org)).
...
# Artikkelin Kuvio 4: YD-kuolemansyyt yhdessä ja muut kuolemansyyt

Haetaan itsemurhatiedot ilman "Itsemurhat huumeilla (X61,X62,X64/T400-9,T436)" -kategoriaa, jotta itsemurhien kanssa ei tule päällekkäisyyksiä.

```{r}
if (lataa_datasetit_uudelleen) {
  pxweb_query_list <- 
...
if (tallenna_datasetit) {
  saveRDS(statfin_ksyyt_pxt_12d9_ilman_itsemurhia, "prepared_data/statfin_ksyyt_pxt_12d9_ilman_itsemurhia.rds", compress = "xz")
}
...
Lopullisen artikkelin version mukaisesti:

```{r 3b_yd_kuolemansyyt_vs_muut_filtterointi}
statfin_ksyyt_pxt_12d9_ilman_itsemurhia <- readRDS("prepared_data/statfin_ksyyt_pxt_12d9_ilman_itsemurhia.rds")
...
kaikki_long <- kaikki_long %>% 
  mutate(kuolemansyy = factor(kuolemansyy, levels=c("muut", "yd_kuolemansyy"), labels = c("Kaikki kuolemansyyt \n(pl. YD-syyt)", "YD-kuolemansyyt")))

```

```{r 3b_yd_kuolemansyyt_vs_muut_visualisointi_final, eval = TRUE}
ggplot(kaikki_long, aes(fill = kuolemansyy, y=value, x=as.numeric(vuosi))) +
  geom_bar(position="stack", stat="identity", color = "black") + 
  labs(x = "Vuosi",
       y = "",
...
### Kuvio 4: Datan tallennus exceliin

```{r, eval = kirjoita_data_excel_tiedostoihin}
kaikki_wide_exceliin <- kaikki_wide %>% 
  dplyr::select(vuosi, muut, yd_kuolemansyy)

if (kirjoita_data_excel_tiedostoihin) {
  write.xlsx(kaikki_wide_exceliin,
           file = "prepared_data/yd_artikkeli_kuviot.xlsx",
           sheetName = "Kuvio_4_yd_yhteensa", 
           append = TRUE)
}
```

# Kuvio 4: Stacked bar plotin tuottaminen ja tallennus pdf-tiedostoksi

Lisätietoa ggstats-paketista: https://larmarange.github.io/ggstats/articles/stat_prop.html

```{r}
library(scales)
library(ggstats)

kuvio4 <- ggplot(kaikki_long) + 
  ...
  theme_bw(base_size = 8, base_family = "sans") +
  theme(legend.position = "bottom", axis.title.y = element_text(angle = 0, vjust = 0.5))

kuvio4 <- kuvio4 + geom_text(
...
if (tuota_vektorikuviot) {
  ggsave("prepared_plots/kuvio4_13x8.pdf", plot = kuvio4, width = 130, height = 80, units = "mm", dpi = 300, scale = 1)
}
```
...
# Artikkelin kuvio 5: ASDR-kuolleisuus

```{r 4_yd_kuolemansyyt_vs_muut_1970_visualisointi_asdr_final_bw, eval = TRUE}
kuvio5 <- ggplot(data = kaikki_70_asdr, aes(x = as.numeric(vuosi), y = asdr, group = kuolemansyy)) +
...
if (tuota_vektorikuviot) {
  # ggsave("prepared_plots/kuvio5_8x5.pdf", width = 80, height = 50, units = "mm", dpi = 300, scale = 1)
  ggsave("prepared_plots/kuvio5_13x8.pdf", plot = kuvio5, width = 130, height = 80, units = "mm", dpi = 300, scale = 1)
}
```

# Extra 1: ASDR-kuolleisuus, YD-kuolemat viivoina ja muut kuolemat palkkeina

```{r extra_visualisoinnit_asdr_yd_kuolemat_ja_muut_erikseen}
kaikki_70_yd <- kaikki_70_asdr %>% 
  filter(kuolemansyy != "Kaikki kuolemansyyt \n(ml. YD-syyt)")

kaikki_70_muut <- kaikki_70_asdr %>% 
  filter(kuolemansyy == "Kaikki kuolemansyyt \n(ml. YD-syyt)")

kuvio_extra_1a <- ggplot(data = kaikki_70_yd, aes(x = as.numeric(vuosi), y = asdr, group = kuolemansyy)) +
  geom_line(aes(color=kuolemansyy, linetype=kuolemansyy), linewidth = 1.5/.pt) + 
  labs(x = "Vuosi",
       y = "",
       colour = "Kuolemansyy",
       linetype = "Kuolemansyy",
       title = "(1a)") +
  scale_colour_grey() +
  theme_bw(base_size = 8, base_family = "sans") +
  theme(legend.position = "bottom") +
...
kuvio_extra_1b <- ggplot(data = kaikki_70_muut, aes(x = as.numeric(vuosi), y = asdr, group = kuolemansyy)) +
  geom_bar(aes(fill=kuolemansyy), stat = "identity", width = 0.5) + 
  labs(x = "Vuosi",
       y = "",
       fill = "Kuolemansyy",
       title = "(1b)") +
...
if (tuota_vektorikuviot) {
  ggsave("prepared_plots/extras/kuvio_extra_1a_13x8.pdf", plot = kuvio_extra_1a, width = 130, height = 80, units = "mm", dpi = 300, scale = 1)
  ggsave("prepared_plots/extras/kuvio_extra_1b_13x8.pdf", plot = kuvio_extra_1b, width = 130, height = 80, units = "mm", dpi = 300, scale = 1)
}

kuvio_extra_1a + kuvio_extra_1b
```

# Extra 2: Menetetyt elinvuodet 2006-2024
...
Elinajanodotteita on saatavilla vain vuodesta 1986 eteenpäin ja tietoa huumekuolemista vain vuodesta 2006 eteenpäin, joten pääsääntöisesti tässä käytetään rajausta 2006-2024. Olen kuitenkin yleisen mielenkiinnon vuoksi sisällyttänyt tähän dataa itsemurhista ja väkivaltakuolemista vuodesta 1986 eteenpäin, koska näillekin vuosille voidaan laskea menetettyjen elinvuosien määrä.
...
menetetyt_elinvuodet_itsemurha <- left_join(itsemurhat, kuolleet_12ap, by = join_by(vuosi, sukupuoli, "ika" == "ikaluokka")) %>% 
  mutate(menetetyt_elinvuodet = kuolleet_06_24 * elinajanodote_vuosia_mean)
...
ggplot(data = menetetyt_elinvuodet_itsemurha, aes(x = as.numeric(vuosi), y = menetetyt_elinvuodet)) +
  ...
  labs(title = "Menetettyjen elinvuosien määrä ikäryhmittäin ja sukupuolittain",
       subtitle = "Kuolemansyy: itsemurha, 1986-2024",
...
ggplot(data = menetetyt_elinvuodet_vakivalta, aes(x = as.numeric(vuosi), y = menetetyt_elinvuodet)) +
  geom_line(aes(color=interaction(ika, sukupuoli))) + 
  labs(title = "Menetettyjen elinvuosien määrä ikäryhmittäin ja sukupuolittain",
       subtitle = "Kuolemansyy: väkivalta, 1986-2024",
...
ggplot(data = menetetyt_elinvuodet_huumekuolema, aes(x = as.numeric(vuosi), y = menetetyt_elinvuodet)) +
  geom_line(aes(color=interaction(ika, sukupuoli))) + 
  labs(title = "Menetettyjen elinvuosien määrä ikäryhmittäin ja sukupuolittain",
       subtitle = "Kuolemansyy: huumekuolema, 2006-2024",
...
print(citation("xlsx"), bibtex = FALSE)

print(citation("gt"), bibtex = FALSE)

print(citation("knitr"), bibtex = FALSE)

print(citation("kableExtra"), bibtex = FALSE)

print(citation("RColorBrewer"), bibtex = FALSE)

print(citation("ggthemes"), bibtex = FALSE)

print(citation("ggstats"), bibtex = FALSE)

print(citation("devEMF"), bibtex = FALSE)
```
